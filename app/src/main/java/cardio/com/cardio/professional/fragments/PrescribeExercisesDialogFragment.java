...
public class PrescribeExercisesDialogFragment extends  android.support.v4.app.DialogFragment {
    ...
    private void saveIntoFirebase(Recomentation recomentation){
        ...
        try {
            ...
            mDbRef.child(recomentation.getId()).child(FirebaseHelper.QUANTITY_KEY).setValue(((Exercicio) recomentation.getAction()).getExercise());
            mDbRef.child(recomentation.getId()).child(FirebaseHelper.QUANTITY_KEY).setValue(((Exercicio) recomentation.getAction()).getIntensity());
            mDbRef.child(recomentation.getId()).child(FirebaseHelper.QUANTITY_KEY).setValue(((Exercicio) recomentation.getAction()).getDuration());
